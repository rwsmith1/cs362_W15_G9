...
class databaseEvent(Database):
    ...
    # Parameter: db = database object
    ####################################################################
    def addApp(self, db, userName, studentName, timeStart, date, location, uId, canceled=0):
        # self.Database()
        # self.connect()
        # userName = appObj.
        # studentName = appObj.
        ...
        # timeStart = appObj.
        # date = appObj.
        self.sql = "INSERT INTO Appointment (fkUser,fkStudent, timeStart, data, location, canceled, uId) VALUES ((SELECT pkUser FROM User WHERE name = '%s'),(SELECT pkStudent FROM Student WHERE name = '%s'),'%s','%s','%s','%d','%s')" % (userName, studentName, timeStart, date, location, canceled, uId)
        # return self.sql
        db.update(self.sql)
